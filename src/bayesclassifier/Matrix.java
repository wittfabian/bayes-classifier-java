...
public class Matrix {
    ...
    public void writeFile(String outfile){
        ...
        try (PrintWriter writer = new PrintWriter(outfile, "UTF-8")) {
            ...
            while(iter.hasNext()){
                String aktIter = iter.next();
                writer.print(aktIter + ";");
            } 
            writer.print("\n");
            for(int i = 0; i < m_matrix.length; i++){
                writer.print(m_classes.get(i) + ";");
                for(int j = 0; j < m_matrix[i].length; j++){
                    if(i == j)
                        m_correctClass += m_matrix[i][j];
                    writer.print(Math.round(m_matrix[i][j]/m_xval) + ";");
                }
                writer.print("\n");
            }
            writer.print("\n");
            writer.println("correct classified: " + Math.round((double)m_correctClass / m_data.size()*10000.0)/100.0 + "%");
